...
// See migration 032_fix_unshare_group_file_cleanup.sql.
//
...
const FIXED_FUNCTION_SQL = readFileSync(
	join(DIRNAME, 'migrations', '032_fix_unshare_group_file_cleanup.sql'),
	'utf8'
)
...
describeMaybe('delete_file_states trigger (unshare cleanup)', () => {
	...
	describe('with the fixed function (migration 032)', () => {
		beforeEach(async () => {
			await pool.query(`SET search_path TO "${schemaName}"`)
