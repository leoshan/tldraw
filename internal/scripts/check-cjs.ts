 ...
 * `ERR_REQUIRE_ESM` throw even on newer Node). Any ESM-only dep that is not
 * already inlined via `ESM_ONLY` shows up as a leak.
 *
 * Known, pre-existing leaks are recorded in `cjs-leak-baseline.json`. The check
 * fails on any NEW leak, and also on any baseline entry that no longer leaks
 * (so the baseline ratchets down as deps get inlined).
 */
...
const BASELINE_PATH = path.join(REPO_ROOT, 'internal', 'scripts', 'cjs-leak-baseline.json')
...
function readBaseline(): Leak[] {
	if (!existsSync(BASELINE_PATH)) return []
	return JSON.parse(readFileSync(BASELINE_PATH, 'utf8')).leaks ?? []
}

function leakKey(leak: Leak): string {
	return `${leak.package} ${leak.specifier}`
}
...
async function main() {
	...
	const leaks: Array<Leak & { files: string[] }> = []
	candidateList.forEach((candidate, id) => {
		const code = codes.get(id)
		if (code && ESM_REQUIRE_ERROR_CODES.has(code)) {
			leaks.push({
				package: candidate.packageName,
				specifier: candidate.specifier,
				files: [...candidate.files].sort(),
			})
		}
	})

	const baseline = readBaseline()
	const baselineKeys = new Set(baseline.map(leakKey))
	const actualKeys = new Set(leaks.map(leakKey))

	const newLeaks = leaks.filter((leak) => !baselineKeys.has(leakKey(leak)))
	const fixedBaselineEntries = baseline.filter((leak) => !actualKeys.has(leakKey(leak)))

	let failed = false

	if (newLeaks.length > 0) {
		failed = true
		console.error(`\nFound ${newLeaks.length} new ESM-only dependency leak(s) in the CJS build:\n`)
		for (const leak of newLeaks) {
			console.error(`  ${leak.package} require()s ESM-only "${leak.specifier}"`)
			for (const file of leak.files) console.error(`    - ${file}`)
		}
		console.error(
			`\nThese throw ERR_REQUIRE_ESM for CJS consumers (Node <20.19, Jest, ts-node).` +
				`\nInline the dependency by adding its package name to ESM_ONLY in` +
				`\ninternal/scripts/build-package.ts (route multi-site deps through a vendor` +
				`\nshim first, like the no-tiptap-default-import rule). Do not add new entries to` +
				`\nthe baseline to silence this.\n`
		)
	}

	if (fixedBaselineEntries.length > 0) {
		failed = true
		console.error(
			`\n${fixedBaselineEntries.length} baseline entry/entries no longer leak - remove them from` +
				`\n${path.relative(REPO_ROOT, BASELINE_PATH)}:\n`
		)
		for (const leak of fixedBaselineEntries) {
			console.error(`  ${leak.package} -> ${leak.specifier}`)
		}
		console.error('')
	}
	...
	const baselineNote =
		baseline.length > 0 ? ` (${baseline.length} known leak(s) in the baseline)` : ''
	nicelog(`No new ESM-only dependency leaks in the CJS build${baselineNote}.`)
}
